...
def _build_report_html_str(study_result_obj: CapacityStudyResult) -> str:
    ...
    extrapolation_warning_html_str = (
        "<p><b>Academic extrapolation warning:</b> some common-stock MOO orders "
        "exceed 1% ADV, outside the supported profile range. Treat those AUM results "
        "as diagnostic only.</p>"
        if execution_policy_str == MOO_EXECUTION_POLICY_STR
    ...
    proxy_extrapolation_warning_html_str = (
        "<p><b>ETF proxy extrapolation warning:</b> some ETF MOO orders exceed 1% ADV. "
        "The common-stock-derived coefficient has no empirical support at those sizes; treat "
        "those AUM results as diagnostic only.</p>"
        if bool(profile_assumption_dict["proxy_bool"])
        and _is_finite_float(proxy_extrapolation_share_float)
        and proxy_extrapolation_share_float > 0.0
        else ""
    )
    # *** CRITICAL*** These two flags say the AUM results are diagnostic only.
    # They used to sit as list items inside the standing-assumptions block; now
    # that block is collapsed by default, a run-specific warning left in there
    # would be hidden behind a click on exactly the runs that need it loudest.
