...
class CapacityStudyResult:
    ...
    order_diagnostics_df: pd.DataFrame
    summary_dict: dict[str, object]
    equity_curve_by_aum_dict: dict[float, pd.DataFrame]
...
def build_capacity_study_result(
    capacity_run_result_list: list[CapacityRunResult],
    output_dir_str: str = "results",
    save_output_bool: bool = True,
) -> CapacityStudyResult:
    if not capacity_run_result_list:
        raise ValueError("capacity_run_result_list must not be empty.")

    strategy_name_set = {result_obj.strategy_name_str for result_obj in capacity_run_result_list}
    policy_set = {result_obj.execution_policy_str for result_obj in capacity_run_result_list}
    profile_set = {result_obj.impact_profile_str for result_obj in capacity_run_result_list}
    ...
    curve_row_dict_list = [dict(result_obj.summary_dict) for result_obj in sorted_result_list]
    capacity_curve_df = pd.DataFrame(curve_row_dict_list).sort_values("capital_base_float")
    order_diagnostics_df = pd.concat(
        [
    ...
    strategy_name_str = sorted_result_list[0].strategy_name_str
    execution_policy_str = sorted_result_list[0].execution_policy_str
    impact_profile_str = sorted_result_list[0].impact_profile_str
    summary_dict = _build_study_summary_dict(
    ...
    study_result_obj = CapacityStudyResult(
        ...
        order_diagnostics_df=order_diagnostics_df,
        summary_dict=summary_dict,
        equity_curve_by_aum_dict={
            result_obj.capital_base_float: result_obj.equity_curve_df
            for result_obj in sorted_result_list
        },
    )
    if save_output_bool:
...
def _build_order_diagnostics_df(
    ...
    for _, order_ser in aggregated_order_df.iterrows():
        ...
        academic_extrapolation_bool = bool(
            assessed_bool
...
def _build_run_summary_tuple(
    ...
    baseline_active_return_float = _active_return_float(
        baseline_annual_return_float,
        benchmark_annual_return_float,
    )
    central_active_return_float = _active_return_float(
        central_annual_return_float,
        benchmark_annual_return_float,
    )
    stress_active_return_float = _active_return_float(
    ...
    rolling_erosion_float = _worst_positive_rolling_sharpe_erosion_float(
        baseline_equity_ser,
        central_equity_ser,
    )
    profile_assumption_dict = impact_profile_assumption_dict(
    ...
    summary_dict = {
        ...
        "baseline_active_return_float": baseline_active_return_float,
        "central_active_return_float": central_active_return_float,
        "stress_active_return_float": stress_active_return_float,
        ...
        "central_cost_consumption_float": _cost_consumption_float(
            baseline_active_return_float,
            central_active_return_float,
        ),
        "stress_cost_consumption_float": _cost_consumption_float(
            baseline_active_return_float,
            stress_active_return_float,
        ),
        "worst_positive_rolling_3y_sharpe_erosion_float": rolling_erosion_float,
        "rolling_3y_available_bool": _is_finite_float(rolling_erosion_float),
...
def _build_study_summary_dict(
    ...
    curve_df["recommended_pass_bool"] = (
        ...
        & (
            curve_df["central_cost_consumption_float"]
            <= RECOMMENDED_COST_CONSUMPTION_LIMIT_FLOAT
        )
        & (curve_df["rolling_3y_available_bool"] == True)
        & (
            curve_df["worst_positive_rolling_3y_sharpe_erosion_float"]
            <= RECOMMENDED_SHARPE_EROSION_LIMIT_FLOAT
        )
    )
    curve_df["outer_pass_bool"] = (
        (curve_df["liquidity_complete_bool"] == True)
        & (curve_df["stress_active_return_float"] > 0.0)
        & (curve_df["hard_breach_share_float"] < OUTER_HARD_BREACH_SHARE_LIMIT_FLOAT)
        & (
            curve_df["stress_cost_consumption_float"]
    ...
    for column_str in ["recommended_pass_bool", "outer_pass_bool"]:
        capacity_curve_df[column_str] = curve_df[column_str].astype(bool).values

    recommended_capacity_float = _highest_passing_aum_float(
        curve_df,
        "recommended_pass_bool",
    )
    outer_capacity_float = _highest_passing_aum_float(curve_df, "outer_pass_bool")
    ...
    return {
        ...
        "recommended_rule_str": (
            ...
            "positive-window rolling 3y Sharpe erosion <= 20%, and incremental cost "
            "<= 25% of active return."
        ),
        "outer_rule_str": (
            "Hard breaches below 5% plus positive stress active return and stress cost "
            "below 50% of active return."
        ),
...
def _worst_positive_rolling_sharpe_erosion_float(
    baseline_equity_ser: pd.Series,
    ...
    positive_baseline_ser = baseline_sharpe_ser[baseline_sharpe_ser > 0.0]
    if len(positive_baseline_ser) == 0:
        return np.nan
    aligned_central_ser = central_sharpe_ser.reindex(positive_baseline_ser.index)
    erosion_ser = (
        (positive_baseline_ser - aligned_central_ser) / positive_baseline_ser.abs()
    ).clip(lower=0.0)
    erosion_ser = erosion_ser.replace([np.inf, -np.inf], np.nan).dropna()
    return float(erosion_ser.max()) if len(erosion_ser) else np.nan


def _active_return_float(strategy_return_float: float, benchmark_return_float: float) -> float:
...
def _highest_passing_aum_float(curve_df: pd.DataFrame, column_str: str) -> float | None:
    passing_df = curve_df[curve_df[column_str] == True]
    if len(passing_df) == 0:
        return None
    return float(passing_df["capital_base_float"].max())
...
def _break_even_bracket_str(curve_df: pd.DataFrame) -> str:
    ...
    for _, row_ser in sorted_df.iterrows():
        current_pair = (
            ...
            _coerce_float(row_ser["central_active_return_float"]),
        )
    ...
    sign_set = {1 if active_return_float > 0.0 else -1 for _, active_return_float in finite_pair_list}
    if sign_set == {1}:
...
def _build_report_html_str(study_result_obj: CapacityStudyResult) -> str:
    ...
    recommended_capacity_str = _fmt_optional_dollar_str(
        summary_dict.get("recommended_capacity_float")
    )
    outer_capacity_str = _fmt_optional_dollar_str(summary_dict.get("outer_capacity_float"))
    ...
    read_first_str = (
        f"The highest fully supported grid point is {recommended_capacity_str}. "
    ...
    bottleneck_html_str = _bottleneck_table_html_str(study_result_obj.order_diagnostics_df)
...
<section class="panel read-first"><h2>Read this first</h2><p>{html.escape(read_first_str)}</p><p>{html.escape(policy_explanation_str)}</p></section>
<section class="cards"><div class="card">Optimal capacity<b>{optimal_capacity_str}</b><span class="muted">Highest Central Sharpe among supported grid points</span></div><div class="card">Recommended max<b>{recommended_capacity_str}</b><span class="muted">Highest deployable grid point meeting every central rule</span></div><div class="card">Outer capacity<b>{outer_capacity_str}</b><span class="muted">Stretch estimate, not an operating target</span></div><div class="card">Break-even bracket<b>{break_even_str}</b><span class="muted">Central active-alpha crossing on the tested grid</span></div></section>
<h2>Visual capacity curve</h2><section class="charts"><div class="panel">{performance_chart_str}</div><div class="panel">{equity_chart_str}<p class="muted">{html.escape(equity_chart_note_str)}</p></div><div class="panel">{liquidity_chart_str}</div><div class="panel">{breach_chart_str}</div></section>
<section class="panel"><h2>AUM results</h2>{table_html_str}</section>
...
<section class="panel"><h2>Assumptions and limitations</h2><ul><li>One strategy declares one auction policy and every MOO strategy explicitly declares one impact profile. The analyzer does not guess.</li><li>{html.escape(proxy_warning_str)}</li>{extrapolation_warning_html_str}<li>Orders are aggregated only within this strategy by date, asset, and side. Client and pod aggregation is not modeled.</li><li>No partial fills, queue position, routing, auction imbalance, borrow, or live broker behavior is simulated.</li><li>Norgate Close is suitable for MOC research; Norgate Open can differ from an official listing-exchange opening auction print.</li><li>Recommended and Outer capacity are not awarded when any saved order lacks both required lagged ADV measures.</li><li>Capacity is a research estimate. Live TCA should replace or recalibrate these assumptions.</li><li>Unavailable order share across saved rows: {_fmt_pct_str(summary_dict.get('unavailable_order_share_float'))}.</li><li>Academic extrapolation share across saved rows: {_fmt_pct_str(summary_dict.get('academic_extrapolation_share_float'))}.</li></ul><p class="muted">Research anchors: <a href="https://doi.org/10.1017/S0022109026102592">Goyal, Jegadeesh and Wu (2026)</a>; <a href="https://norgatedata.com/data-content-tables.php">Norgate price definitions</a>; <a href="https://www.interactivebrokers.com/en/pricing/commissions-stocks.php">IBKR commissions</a>.</p></section>
...
def _selected_equity_chart_tuple(
    ...
    selected_aum_float = (
        min(study_result_obj.equity_curve_by_aum_dict)
        if diagnostic_only_bool
        else float(recommended_capacity_obj)
    )
    equity_curve_df = study_result_obj.equity_curve_by_aum_dict[selected_aum_float]
...
def _capacity_curve_table_html_str(curve_df: pd.DataFrame, execution_policy_str: str) -> str:
    ...
    for _, row_ser in curve_df.iterrows():
        ...
        row_html_list.append(
            ...
            f"<td>{_fmt_pct_str(row_ser['central_cost_consumption_float'])}</td>"
    ...
    return (
        ...
        '<th>P99 Order/ADV</th><th>Cost / active return</th><th>Hard breaches</th>'
