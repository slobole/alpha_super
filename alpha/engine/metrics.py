...
def select_tail_event_date_index(
    portfolio_daily_return_ser: pd.Series,
    tail_fraction_float: float = 0.05,
    min_tail_days_int: int = 1,
) -> pd.DatetimeIndex:
    """Select the worst realized portfolio-return dates."""
    if tail_fraction_float <= 0.0 or tail_fraction_float > 1.0:
        raise ValueError(f"tail_fraction_float must be in (0, 1], got {tail_fraction_float}.")
    if min_tail_days_int <= 0:
        raise ValueError(f"min_tail_days_int must be positive, got {min_tail_days_int}.")

    realized_return_ser = portfolio_daily_return_ser.astype(float).replace([np.inf, -np.inf], np.nan).dropna()
    if len(realized_return_ser) == 0:
        return pd.DatetimeIndex([])

    # *** CRITICAL*** Exclude the bootstrap return row. That first row is a
    # capital anchor, not a realized return generated by portfolio risk.
    realized_return_ser = realized_return_ser.iloc[1:]
    if len(realized_return_ser) == 0:
        return pd.DatetimeIndex([])

    tail_day_count_int = int(np.ceil(float(len(realized_return_ser)) * float(tail_fraction_float)))
    tail_day_count_int = max(int(min_tail_days_int), tail_day_count_int)
    tail_day_count_int = min(tail_day_count_int, len(realized_return_ser))

    tail_event_date_index = realized_return_ser.sort_values(kind='mergesort').iloc[
        :tail_day_count_int
    ].index
    return pd.DatetimeIndex(tail_event_date_index).sort_values()


def build_tail_contribution_df(
    pod_daily_return_df: pd.DataFrame,
    pod_equity_df: pd.DataFrame,
    tail_event_date_index: pd.DatetimeIndex,
) -> pd.DataFrame:
    """
    Compute pod return contributions on portfolio tail days.

    For pod i on date t:

        contribution_{i,t} = weight_{i,t-1} * return_{i,t}

    where weight_{i,t-1} is prior close pod equity divided by prior close
    portfolio equity.
    """
    common_index = pod_daily_return_df.index.intersection(pod_equity_df.index)
    common_column_list = [
        column_str for column_str in pod_daily_return_df.columns
        if column_str in pod_equity_df.columns
    ]
    if len(common_index) == 0 or len(common_column_list) == 0 or len(tail_event_date_index) == 0:
        return pd.DataFrame(index=tail_event_date_index, columns=common_column_list, dtype=float)

    aligned_return_df = pod_daily_return_df.loc[common_index, common_column_list].astype(float)
    aligned_equity_df = pod_equity_df.loc[common_index, common_column_list].astype(float)

    # *** CRITICAL*** Tail contribution must use capital known before the
    # return day. Same-day ending weights would leak the realized tail move
    # into the attribution denominator.
    previous_equity_df = aligned_equity_df.shift(1)
    previous_portfolio_equity_ser = previous_equity_df.sum(axis=1)
    previous_weight_df = previous_equity_df.div(previous_portfolio_equity_ser, axis=0)
    contribution_df = previous_weight_df * aligned_return_df

    valid_tail_index = pd.DatetimeIndex(tail_event_date_index).intersection(contribution_df.index)
    return contribution_df.loc[valid_tail_index]


def build_tail_summary_df(
    pod_tail_return_df: pd.DataFrame,
    tail_contribution_df: pd.DataFrame,
    portfolio_tail_return_ser: pd.Series,
) -> pd.DataFrame:
    """Summarize pod behavior on portfolio tail days."""
    if len(pod_tail_return_df) == 0:
        return pd.DataFrame(
            columns=[
                'average_tail_return_float',
                'worst_tail_return_float',
                'negative_tail_day_rate_float',
                'average_tail_contribution_float',
                'worst_tail_contribution_float',
                'average_loss_contribution_share_float',
            ],
            dtype=float,
        )

    aligned_portfolio_tail_return_ser = portfolio_tail_return_ser.reindex(
        tail_contribution_df.index
    ).astype(float)
    loss_denominator_ser = aligned_portfolio_tail_return_ser.where(
        aligned_portfolio_tail_return_ser < 0.0,
        np.nan,
    )
    loss_contribution_share_df = tail_contribution_df.div(loss_denominator_ser, axis=0)

    tail_summary_df = pd.DataFrame(index=pod_tail_return_df.columns, dtype=float)
    tail_summary_df['average_tail_return_float'] = pod_tail_return_df.mean()
    tail_summary_df['worst_tail_return_float'] = pod_tail_return_df.min()
    tail_summary_df['negative_tail_day_rate_float'] = (pod_tail_return_df < 0.0).mean()
    tail_summary_df['average_tail_contribution_float'] = tail_contribution_df.mean()
    tail_summary_df['worst_tail_contribution_float'] = tail_contribution_df.min()
    tail_summary_df['average_loss_contribution_share_float'] = loss_contribution_share_df.mean()
    tail_summary_df.index.name = 'pod_name_str'
    return tail_summary_df


def generate_tail_risk_diagnostics(
    pod_daily_return_df: pd.DataFrame,
    portfolio_daily_return_ser: pd.Series,
    pod_equity_df: pd.DataFrame,
    tail_fraction_float: float = 0.05,
    ...
    if len(tail_event_date_index) == 0 or len(common_column_list) == 0:
        empty_return_df = pd.DataFrame(index=tail_event_date_index, columns=common_column_list, dtype=float)
        empty_correlation_df = cross_correlation_matrix(empty_return_df)
        ...
        return {
            ...
            'tail_summary_df': build_tail_summary_df(
                empty_return_df,
                empty_contribution_df,
    ...
    tail_summary_df = build_tail_summary_df(
        pod_tail_return_df=tail_return_df,
        tail_contribution_df=tail_contribution_df,
        portfolio_tail_return_ser=portfolio_daily_return_ser.loc[tail_event_date_index],
    )
