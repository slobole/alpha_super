"""
Research-only sector-dispersion IBS variant with KIE, IHI, and XLC added.

This is the runnable strategy surface for the lower-tail-beta finalist from
the combination and market-correlation studies:
...
STRATEGY_NAME_STR = "strategy_mr_sector_dispersion_ibs_kie_ihi_xlc"
STRATEGY_SYMBOL_TUPLE = ORIGINAL_SYMBOL_TUPLE + ("KIE", "IHI", "XLC")


class SectorDispersionIbsKieIhiXlcStrategy(SectorDispersionIbsStrategy):
    """Metadata wrapper for the fixed KIE+IHI+XLC basket variant."""
...
__all__ = [
    "DEFAULT_CONFIG",
    "SectorDispersionIbsKieIhiXlcStrategy",
    "STRATEGY_NAME_STR",
...
def build_capacity_analysis_inputs(
    ...
    return build_sector_dispersion_capacity_analysis_inputs(
        strategy_class=SectorDispersionIbsKieIhiXlcStrategy,
...
def _write_kie_ihi_xlc_variant_notes_md(output_path: Path) -> None:
    notes_md_str = f"""# Sector Dispersion IBS KIE+IHI+XLC Variant
...
- This is the defensive/lower-tail-beta finalist from the combination and market-correlation studies.
...
"""
    (output_path / "sector_dispersion_ibs_kie_ihi_xlc_variant_notes.md").write_text(
        notes_md_str,
        encoding="utf-8",
    )
...
def run_variant(
    ...
    strategy_obj = SectorDispersionIbsKieIhiXlcStrategy(
    ...
    if save_results_bool:
        ...
        _write_kie_ihi_xlc_variant_notes_md(output_path=output_path)
