"""
Research-only sector-dispersion IBS variant with KIE and IHI added.

This is the runnable strategy surface for the practical default from the
combination and market-correlation studies:
...
STRATEGY_NAME_STR = "strategy_mr_sector_dispersion_ibs_kie_ihi"
STRATEGY_SYMBOL_TUPLE = ORIGINAL_SYMBOL_TUPLE + ("KIE", "IHI")


class SectorDispersionIbsKieIhiStrategy(SectorDispersionIbsStrategy):
    """Metadata wrapper for the fixed KIE+IHI basket variant."""
...
def _write_kie_ihi_variant_notes_md(output_path: Path) -> None:
    notes_md_str = f"""# Sector Dispersion IBS KIE+IHI Variant
...
- This is the practical default from the combination and market-correlation studies.
...
"""
    (output_path / "sector_dispersion_ibs_kie_ihi_variant_notes.md").write_text(
        notes_md_str,
        encoding="utf-8",
    )
...
def run_variant(
    ...
    strategy_obj = SectorDispersionIbsKieIhiStrategy(
    ...
    if save_results_bool:
        ...
        _write_kie_ihi_variant_notes_md(output_path=output_path)
