"""
Research-only Russell 3000 smooth-trend N=10 long/short variant.

This keeps the same Option A construction as the N=20 long/short baseline but
uses a more concentrated selected corner breadth of 10 longs and 10 shorts.
"""
...
DEFAULT_CONFIG = replace(
    ...
    variant_key_str="russell3000_option_a_n10_long_short_close_gt_1",
    max_long_positions_int=10,
    max_short_positions_int=10,
)
...
class SmoothTrendRussell3000N10LongShortStrategy(SmoothTrendRussell3000LongShortStrategy):
    """
    Research-only monthly Russell 3000 Option A N=10 long/short selector.
    """


def _write_assumptions_md(
    output_path: Path,
    strategy_obj: SmoothTrendRussell3000N10LongShortStrategy,
) -> None:
    config_obj = strategy_obj.config
    assumption_md_str = f"""# Smooth Trend Russell 3000 N=10 Long/Short Assumptions
...
"""
    (output_path / "smooth_trend_russell3000_n10_long_short_assumptions.md").write_text(
        assumption_md_str,
        encoding="utf-8",
    )
...
def run_variant(
    ...
    strategy_obj = SmoothTrendRussell3000N10LongShortStrategy(
        name="strategy_mo_smooth_trend_russell3000_n10_long_short",
