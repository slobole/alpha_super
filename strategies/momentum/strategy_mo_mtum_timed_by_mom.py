"""
MTUM timing strategy driven by mean-reversion in the MOM factor ETF.

Core formulas
-------------
For the signal proxy close series P_t from `MOM-202103`:
...
This file keeps the implementation explicit and audit-friendly. The signal proxy
is loaded from the delisted Norgate symbol `MOM-202103`, which makes the
article-faithful sample end on 2021-03-12.
"""
...
SUPPORTED_SHARED_END_DATE_STR = "2021-03-12"
...
class MtumTimedByMomConfig:
    trade_symbol_str: str = "MTUM"
    signal_symbol_str: str = "MOM-202103"
    benchmark_list: tuple[str, ...] = ("$SPX",)
    start_date_str: str = SUPPORTED_SHARED_START_DATE_STR
    end_date_str: str | None = SUPPORTED_SHARED_END_DATE_STR
...
DEFAULT_CONFIG = MtumTimedByMomConfig()
...
def _resolve_requested_end_date_ts(config: MtumTimedByMomConfig) -> pd.Timestamp:
    if config.end_date_str is None:
        return pd.Timestamp(SUPPORTED_SHARED_END_DATE_STR)
    return pd.Timestamp(config.end_date_str)


def validate_requested_window(config: MtumTimedByMomConfig) -> tuple[pd.Timestamp, pd.Timestamp]:
    ...
    supported_end_date_ts = pd.Timestamp(SUPPORTED_SHARED_END_DATE_STR)
    ...
    if requested_start_date_ts < supported_start_date_ts:
        raise ValueError(
            "Requested start_date_str lies outside the supported MTUM/MOM shared window. "
            f"Expected start_date_str >= {SUPPORTED_SHARED_START_DATE_STR}."
        )
    if requested_end_date_ts > supported_end_date_ts:
        raise ValueError(
            "Requested end_date_str lies outside the supported MTUM/MOM shared window. "
            f"Expected end_date_str <= {SUPPORTED_SHARED_END_DATE_STR}."
        )
...
def load_signal_close_ser(
    ...
    if len(signal_price_df) == 0:
        raise RuntimeError(f"No signal price data was loaded for {signal_symbol_str}.")
...
def merge_signal_close_into_pricing_data_df(
    ...
    if aligned_signal_close_ser.isna().any():
...
def get_mtum_timed_by_mom_data(
    config: MtumTimedByMomConfig = DEFAULT_CONFIG,
) -> pd.DataFrame:
    ...
    execution_price_df = load_execution_price_df(
        ...
        end_date_str=requested_end_date_ts.strftime("%Y-%m-%d"),
    )
    signal_close_ser = load_signal_close_ser(
    ...
    pricing_data_df = merge_signal_close_into_pricing_data_df(
        ...
        signal_symbol_str=config.signal_symbol_str,
        signal_close_ser=signal_close_ser,
    )
    return pricing_data_df


def compute_mom_proxy_signal_df(
    signal_close_ser: pd.Series,
    config: MtumTimedByMomConfig = DEFAULT_CONFIG,
) -> pd.DataFrame:
    ...
    # current signal never references future proxy closes.
    proxy_return_10d_ser = (
        signal_close_ser
        / signal_close_ser.shift(config.signal_return_lookback_day_int)
    ) - 1.0
    ...
    proxy_rank_pct_ser = pd.Series(np.nan, index=signal_close_ser.index, dtype=float)
    ...
    target_weight_ser = pd.Series(np.nan, index=signal_close_ser.index, dtype=float)
...
class MtumTimedByMomStrategy(Strategy):
    """
    Long/flat MTUM strategy gated by mean-reversion in the MOM factor proxy.
    ...
    def _signal_config(self) -> MtumTimedByMomConfig:
        return MtumTimedByMomConfig(
    ...
    def compute_signals(self, pricing_data_df: pd.DataFrame) -> pd.DataFrame:
        signal_close_key = (self.signal_symbol_str, "SignalClose")
        signal_fallback_close_key = (self.signal_symbol_str, "Close")
        trade_close_key = (self.trade_symbol_str, "Close")

        if signal_close_key in pricing_data_df.columns:
            signal_close_ser = pricing_data_df.loc[:, signal_close_key].astype(float)
        elif signal_fallback_close_key in pricing_data_df.columns:
            signal_close_ser = pricing_data_df.loc[:, signal_fallback_close_key].astype(float)
        else:
            raise RuntimeError(
                f"Missing SignalClose/Close history for signal symbol {self.signal_symbol_str}."
            )
        ...
        signal_feature_df = compute_mom_proxy_signal_df(
            signal_close_ser=signal_close_ser,
...
if __name__ == "__main__":
    ...
    pricing_data_df = get_mtum_timed_by_mom_data(config=config)

    strategy = MtumTimedByMomStrategy(
        name="strategy_mo_mtum_timed_by_mom",
