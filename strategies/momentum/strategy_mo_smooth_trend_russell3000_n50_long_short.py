"""
Research-only Russell 3000 smooth-trend N=50 long/short variant.

This keeps the same Option A construction as the N=20 long/short baseline but
expands the selected corner breadth to 50 longs and 50 shorts.
"""
...
DEFAULT_CONFIG = replace(
    ...
    variant_key_str="russell3000_option_a_n50_long_short_close_gt_1",
...
class SmoothTrendRussell3000N50LongShortStrategy(SmoothTrendRussell3000LongShortStrategy):
    """
    Research-only monthly Russell 3000 Option A N=50 long/short selector.
    """


def _write_assumptions_md(
    output_path: Path,
    strategy_obj: SmoothTrendRussell3000N50LongShortStrategy,
) -> None:
    config_obj = strategy_obj.config
    assumption_md_str = f"""# Smooth Trend Russell 3000 N=50 Long/Short Assumptions
...
"""
    (output_path / "smooth_trend_russell3000_n50_long_short_assumptions.md").write_text(
        assumption_md_str,
        encoding="utf-8",
    )
...
def run_variant(
    ...
    strategy_obj = SmoothTrendRussell3000N50LongShortStrategy(
        name="strategy_mo_smooth_trend_russell3000_n50_long_short",
