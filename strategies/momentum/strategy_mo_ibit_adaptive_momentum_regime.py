"""Research-only IBIT Adaptive Momentum market-regime strategy.

The frozen Adaptive Momentum signal is computed from IBIT total-return closes.
A risk-on state buys IBIT at Open_(T+1); a risk-off state exits IBIT at
Open_(T+1).
...
IBIT_CONFIG = SpyAdaptiveMomentumRegimeConfig(
...
def _validate_ibit_config(config_obj: SpyAdaptiveMomentumRegimeConfig) -> None:
    if config_obj.trade_symbol_str != "IBIT":
        raise ValueError("IBIT strategy requires trade_symbol_str='IBIT'.")
    if config_obj.signal_symbol_str != "IBIT_TR_SIGNAL":
        raise ValueError("IBIT strategy requires signal_symbol_str='IBIT_TR_SIGNAL'.")


def get_ibit_adaptive_momentum_regime_data(
    config_obj: SpyAdaptiveMomentumRegimeConfig = IBIT_CONFIG,
) -> pd.DataFrame:
    """Load IBIT execution prices and IBIT total-return signal prices."""

    _validate_ibit_config(config_obj)
...
class IbitAdaptiveMomentumRegimeStrategy(SpyAdaptiveMomentumRegimeStrategy):
    """Trade IBIT using the frozen Adaptive Momentum state computed on IBIT."""

    def __init__(
        self,
        name: str,
        benchmarks: list[str] | tuple[str, ...],
        config: SpyAdaptiveMomentumRegimeConfig = IBIT_CONFIG,
    ) -> None:
        _validate_ibit_config(config)
...
def run_variant(
    ...
    """Run the research-only IBIT self-signal variant."""

    _validate_ibit_config(config_obj)
    if pricing_data_df is None:
        pricing_data_df = get_ibit_adaptive_momentum_regime_data(config_obj)
    strategy_obj = IbitAdaptiveMomentumRegimeStrategy(
        name="strategy_mo_ibit_adaptive_momentum_regime_research",
...
def _build_analysis_context_dict() -> dict[str, object]:
    ...
    return {
        "strategy_name_str": "strategy_mo_ibit_adaptive_momentum_regime_research",
...
def _build_strategy_obj(context_dict: dict[str, object]) -> IbitAdaptiveMomentumRegimeStrategy:
    ...
    _validate_ibit_config(config_obj)
    return IbitAdaptiveMomentumRegimeStrategy(
...
def build_execution_timing_analysis_inputs() -> dict[str, object]:
    ...
    def strategy_factory_fn() -> IbitAdaptiveMomentumRegimeStrategy:
...
def build_capacity_analysis_inputs(
    ...
    config_obj = replace(
        ...
        backtest_start_date_str=(
            IBIT_CONFIG.backtest_start_date_str
