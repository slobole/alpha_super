...
Paper-faithful base implementation with explicit monthly signal math and
next-open execution.
...
class DefenseFirstStrategy(Strategy):
    ...
    Execution math at rebalance open t:

        account_value_open_t = cash_t + sum_i(position_{i,t-} * open_{i,t})

        target_shares_{i,t}
            = floor(account_value_open_t * target_weight_{i,t} / open_{i,t})

        order_shares_{i,t}
            = target_shares_{i,t} - current_shares_{i,t-}

    Sells are submitted before buys so purchases are funded by same-open
    liquidations as closely as this engine allows.
    """
    ...
    def _compute_account_value_open_float(self, open_price_ser: pd.Series) -> float:
        position_ser = self.get_positions().reindex(self.tradeable_asset_list, fill_value=0).astype(float)
        position_value_float = 0.0

        for asset_str in self.tradeable_asset_list:
            share_count_float = float(position_ser.loc[asset_str])
            if share_count_float == 0.0:
                continue

            open_price_float = float(open_price_ser.loc[asset_str])
            if not np.isfinite(open_price_float) or open_price_float <= 0.0:
                raise RuntimeError(f"Invalid open price for held asset {asset_str} on {self.current_bar}.")

            position_value_float += share_count_float * open_price_float

        return float(self.cash) + position_value_float

    def _compute_target_share_delta_ser(
        self,
        target_weight_ser: pd.Series,
        open_price_ser: pd.Series,
    ) -> pd.Series:
        account_value_open_float = self._compute_account_value_open_float(open_price_ser)
        current_position_ser = self.get_positions().reindex(self.tradeable_asset_list, fill_value=0).astype(float)
        delta_share_ser = pd.Series(0, index=self.tradeable_asset_list, dtype=int)
        ...
        for asset_str in self.tradeable_asset_list:
            ...
            if target_weight_float == 0.0:
                delta_share_ser.loc[asset_str] = -current_share_int
                continue
            ...
            target_share_int = int(np.floor(account_value_open_float * target_weight_float / open_price_float))
            delta_share_ser.loc[asset_str] = target_share_int - current_share_int

        return delta_share_ser
    ...
    def iterate(self, data: pd.DataFrame, close: pd.DataFrame, open_prices: pd.Series):
        ...
        open_price_ser = open_prices.reindex(self.tradeable_asset_list)
        delta_share_ser = self._compute_target_share_delta_ser(target_weight_ser, open_price_ser)

        # Submit sell orders first to make the rebalance funding path explicit.
        for asset_str in self.tradeable_asset_list:
            delta_share_int = int(delta_share_ser.loc[asset_str])
            if delta_share_int >= 0:
                continue

            self.order(asset_str, delta_share_int, trade_id=self.current_trade_map[asset_str])
        ...
        for asset_str in self.tradeable_asset_list:
            ...
            if self.get_position(asset_str) == 0:
                self.trade_id_int += 1
