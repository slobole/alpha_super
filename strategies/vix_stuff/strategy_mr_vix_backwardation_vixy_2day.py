"""
VIXY backwardation event strategy with a fixed 2-day hold.

TL;DR: When the prior close shows VIX backwardation:

    VIX_t > VIX3M_t

buy `VIXY` at the next open and hold for exactly 2 trading days, then exit at
the following open.

Core formulas
-------------
Define the backwardation indicator at close t:

    backwardation_t
        = 1[VIX_t > VIX3M_t]

Let H = 2 be the number of trading days to hold after entry.

Trigger dates are denoted by tau_k and satisfy:

    backwardation_{tau_k} = 1

and:

    tau_k > tau_{k-1} + H

so a new trade is not started while the prior 2-day hold window is still
active or during the immediate flat reset bar that follows it.

For each trigger tau_k, the close-indexed target-weight series is:

    w_t
        = w_alloc * 1[t in {tau_k, tau_k + 1}]

This maps to the engine's next-open execution contract:

    entry at open(tau_k + 1)
    hold through open(tau_k + 2)
    exit at open(tau_k + 3)

Target shares at the execution bar are:

    q_{t+1}^{target}
        = floor(V_t * w_t / O_{t+1}^{VIXY})

where:

    V_t = prior-close total portfolio value
    O_{t+1}^{VIXY} = current-bar open for VIXY
"""
...
class VixBackwardationVixy2DayConfig:
    trade_symbol_str: str = "VIXY"
    vix_symbol_str: str = "$VIX"
    vix3m_symbol_str: str = "$VIX3M"
    benchmark_symbol_str: str = "SPY"
    hold_day_count_int: int = 2
...
DEFAULT_CONFIG = VixBackwardationVixy2DayConfig()


def get_vix_backwardation_vixy_2day_data(
    config: VixBackwardationVixy2DayConfig = DEFAULT_CONFIG,
) -> pd.DataFrame:
...
def compute_vix_backwardation_vixy_2day_signal_df(
    ...
    """
    Compute the backwardation trigger state and the close-indexed target weight
    series used by the next-open execution engine.
    """
    ...
    backwardation_bool_ser = term_spread_ser > 0.0

    entry_signal_bool_ser = pd.Series(False, index=signal_input_df.index, dtype=bool, name="entry_signal_bool_ser")
    target_weight_ser = pd.Series(0.0, index=signal_input_df.index, dtype=float, name="target_weight_ser")
    close_bars_remaining_int_ser = pd.Series(0, index=signal_input_df.index, dtype=int, name="close_bars_remaining_int_ser")
    flat_reset_bool_ser = pd.Series(False, index=signal_input_df.index, dtype=bool, name="flat_reset_bool_ser")

    active_followthrough_close_count_int = 0
    flat_reset_pending_bool = False

    for bar_ts in signal_input_df.index:
        backwardation_bool = bool(backwardation_bool_ser.loc[bar_ts])

        if active_followthrough_close_count_int > 0:
            target_weight_ser.loc[bar_ts] = float(target_weight_float)
            close_bars_remaining_int_ser.loc[bar_ts] = int(active_followthrough_close_count_int)
            active_followthrough_close_count_int -= 1
            if active_followthrough_close_count_int == 0:
                flat_reset_pending_bool = True
            continue

        if flat_reset_pending_bool:
            flat_reset_bool_ser.loc[bar_ts] = True
            flat_reset_pending_bool = False
            continue

        if backwardation_bool:
            entry_signal_bool_ser.loc[bar_ts] = True
            target_weight_ser.loc[bar_ts] = float(target_weight_float)
            active_followthrough_close_count_int = hold_day_count_int - 1
            close_bars_remaining_int_ser.loc[bar_ts] = int(active_followthrough_close_count_int)
    ...
    signal_feature_df = pd.DataFrame(
        {
            ...
            "backwardation_bool": backwardation_bool_ser.astype(bool),
            "entry_signal_bool_ser": entry_signal_bool_ser.astype(bool),
            "flat_reset_bool_ser": flat_reset_bool_ser.astype(bool),
            "close_bars_remaining_int_ser": close_bars_remaining_int_ser.astype(int),
...
class VixBackwardationVixy2DayStrategy(Strategy):
    """
    Single-asset backwardation event sleeve in `VIXY`.
    """
    ...
    def __init__(
        ...
        self.hold_day_count_int = int(hold_day_count_int)
    ...
    def compute_signals(self, pricing_data_df: pd.DataFrame) -> pd.DataFrame:
        ...
        signal_feature_df = compute_vix_backwardation_vixy_2day_signal_df(
            vix_close_ser=signal_data_df[(self.vix_symbol_str, "Close")],
            vix3m_close_ser=signal_data_df[(self.vix3m_symbol_str, "Close")],
            hold_day_count_int=self.hold_day_count_int,
            target_weight_float=self.target_weight_float,
        )

        # *** CRITICAL*** The close-indexed target_weight_ser is constructed
        # causally from current and past backwardation states only. The engine
        # consumes previous_bar features and executes at the current open.
    ...
    def iterate(self, data_df: pd.DataFrame, close_row_ser: pd.Series, open_price_ser: pd.Series):
        ...
        target_weight_key = (SIGNAL_NAMESPACE_STR, "target_weight_ser")
...
if __name__ == "__main__":
    ...
    pricing_data_df = get_vix_backwardation_vixy_2day_data(config=config)
    ...
    strategy = VixBackwardationVixy2DayStrategy(
        name="strategy_mr_vix_backwardation_vixy_2day",
        benchmarks=[config.benchmark_symbol_str],
        trade_symbol_str=config.trade_symbol_str,
