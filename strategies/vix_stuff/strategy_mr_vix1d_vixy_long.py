"""
Long-only VIXY mean-reversion strategy driven by VIX1D extremes.

TL;DR: Buy `VIXY` at the next open when the prior close shows:
1. very low 1-day implied volatility, and
...
At close t:
    ...
    target_weight_t
        = w_alloc * long_signal_t

At the next open t + 1:

    q_{t+1}^{target}
        = floor(V_t * target_weight_t / O_{t+1}^{VIXY})
...
Instrument note
---------------
This is a trade in a volatility ETP, not in spot VIX itself. The economic
exposure is to the VIX futures basket held by `VIXY`, so:
...
DEFAULT_CONFIG = Vix1dVixyLongConfig()


def get_vix1d_vixy_long_data(
    config: Vix1dVixyLongConfig = DEFAULT_CONFIG,
) -> pd.DataFrame:
...
def compute_vix1d_vixy_long_signal_df(
    ...
    """
    Compute the long-only VIXY signal state from the three volatility series.

    The signal is intentionally simple:

        long_signal_t = 1[VIX1D_t <= theta_long] * 1[VIX_t <= VIX3M_t]

    The returned target weight is the decision recorded at close t and intended
    for execution at the next open t + 1 by the engine.
    """
    ...
    signal_feature_df = pd.DataFrame(
        {
            ...
            "long_signal_bool": long_signal_bool_ser.astype(bool),
...
class Vix1dVixyLongStrategy(Strategy):
    """
    Long-only `VIXY` sleeve.

    The signal is observed on the prior close:

        target_weight_t = w_alloc * 1[VIX1D_t <= theta_long] * 1[VIX_t <= VIX3M_t]
    ...
    This preserves the house execution contract:

        prior-close decision -> next-open execution
    """
    ...
    def compute_signals(self, pricing_data_df: pd.DataFrame) -> pd.DataFrame:
        ...
        signal_feature_df = compute_vix1d_vixy_long_signal_df(
...
if __name__ == "__main__":
    ...
    pricing_data_df = get_vix1d_vixy_long_data(config=config)

    strategy = Vix1dVixyLongStrategy(
        name="strategy_mr_vix1d_vixy_long",
        benchmarks=[config.benchmark_symbol_str],
