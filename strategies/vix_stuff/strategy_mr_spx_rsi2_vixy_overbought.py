"""
SPX RSI(2)-driven long-volatility sleeve implemented through long `VIXY`.

TL;DR: Buy `VIXY` at the next open when the prior close shows:
1. `SPX` RSI(2) above 90, and
...
--------------------
This implementation uses a rolling target-position rule:

    if entry_signal_t = 1:
        hold long-vol exposure at open t + 1

    if entry_signal_t = 0:
        target flat at open t + 1

So repeated true signals can chain into a multi-session holding period.

Instrument note
---------------
Long `VIXY` is a tradable proxy for long-vol exposure, but it is not spot VIX.
Its return is linked to a short-term VIX futures basket, so:

    return_t^{VIXY} != Delta VIX_t

in general because of futures roll, carry, fees, and path dependence.
"""
...
DEFAULT_CONFIG = SpxRsi2VixyOverboughtConfig()


def get_spx_rsi2_vixy_overbought_data(
    config: SpxRsi2VixyOverboughtConfig = DEFAULT_CONFIG,
) -> pd.DataFrame:
...
def compute_spx_rsi2_vixy_overbought_signal_df(
    ...
    """
    Compute the SPX RSI(2) overbought signal state for the VIXY sleeve.
...
class SpxRsi2VixyOverboughtStrategy(Strategy):
    """
    Single-asset long-vol sleeve driven by prior-close SPX RSI(2).

        target_weight_t = w_alloc * 1[RSI2_t > theta_rsi]

    and the target shares submitted at the next open are:

        q_{t+1}^{target} = floor(V_t * target_weight_t / O_{t+1}^{VIXY})
    """
    ...
    def compute_signals(self, pricing_data_df: pd.DataFrame) -> pd.DataFrame:
        ...
        signal_feature_df = compute_spx_rsi2_vixy_overbought_signal_df(
...
if __name__ == "__main__":
    ...
    pricing_data_df = get_spx_rsi2_vixy_overbought_data(config=config)
    ...
    strategy = SpxRsi2VixyOverboughtStrategy(
        name="strategy_mr_spx_rsi2_vixy_overbought",
        benchmarks=[config.benchmark_symbol_str],
        trade_symbol_str=config.trade_symbol_str,
