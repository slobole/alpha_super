"""
DV2 long-only mean-reversion strategy with price and ADV filters.
...
from alpha.indicators import adv_dollar_indicator, dv2_indicator
...
class DV2PriceAdvStrategy(Strategy):
    """
    DV2 stock pod with additional price and liquidity eligibility filters.
    ...
    def compute_signals(self, pricing_data: pd.DataFrame) -> pd.DataFrame:
        ...
        prior_close_126_df = close_df.shift(self.return_lookback_days_int)
        return_126_day_df = (close_df / prior_close_126_df) - 1.0
        ...
        natr_value_df = pd.DataFrame(natr_value_map, index=signal_data.index)
        dv2_value_df = pd.DataFrame(dv2_value_map, index=signal_data.index)
    ...
    def get_opportunity_list(self, close_row_ser: pd.Series) -> list[str]:
        ...
        candidate_df = candidate_df[
            candidate_df["dv2_value_ser"].astype(float) < self.dv2_threshold_float
        ]
        candidate_df = candidate_df[
            candidate_df["Close"].astype(float) > candidate_df["sma_200_price_ser"].astype(float)
        ]
        candidate_df = candidate_df[
            candidate_df["return_126_day_ser"].astype(float) > 0.005
        ]
...
if __name__ == "__main__":
    ...
    strategy = DV2PriceAdvStrategy(
        name="strategy_mr_dv2_price_adv",
