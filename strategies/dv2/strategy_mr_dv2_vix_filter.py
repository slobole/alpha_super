...
VIX_SMA_WINDOW_DAY_INT = 15
VIX_BEAR_BUFFER_FLOAT = 0.15
...
def compute_vix_regime_signal_df(vix_close_ser: pd.Series) -> pd.DataFrame:
    """
    Compute the daily VIX bull/bear regime table.

    Formulas:

        vix_sma15_t
            = (1 / 15) * sum_{i=0}^{14} VIX_{t - i}

        vix_bear_threshold_t
            = 1.15 * vix_sma15_t

        vix_bear_bool_t
            = 1[VIX_t > vix_bear_threshold_t]

        vix_bull_bool_t
            = 1 - vix_bear_bool_t
    """
    vix_close_ser = pd.Series(vix_close_ser, copy=True).astype(float).sort_index()
    vix_close_ser.name = "vix_close_ser"

    # *** CRITICAL*** The 15-day SMA must be strictly backward-looking.
    # A centered or forward window would leak future volatility into the
    # current regime classification.
    vix_sma_15_ser = vix_close_ser.rolling(VIX_SMA_WINDOW_DAY_INT).mean()
    vix_bear_threshold_ser = vix_sma_15_ser * (1.0 + VIX_BEAR_BUFFER_FLOAT)

    regime_ready_bool_ser = vix_bear_threshold_ser.notna()
    vix_bear_bool_ser = (vix_close_ser > vix_bear_threshold_ser).astype("boolean")
    vix_bear_bool_ser = vix_bear_bool_ser.where(regime_ready_bool_ser)
    vix_bull_bool_ser = (~vix_bear_bool_ser).astype("boolean")

    vix_regime_signal_df = pd.DataFrame(
        {
            "vix_close_ser": vix_close_ser,
            "vix_sma_15_ser": vix_sma_15_ser,
            "vix_bear_threshold_ser": vix_bear_threshold_ser,
            "vix_bear_bool": vix_bear_bool_ser,
            "vix_bull_bool": vix_bull_bool_ser,
        },
        index=vix_close_ser.index,
    )
    return vix_regime_signal_df


class DV2VixFilterStrategy(Strategy):
    """
    DV2 stock mean-reversion variant with a VIX bull/bear entry gate.
    ...
    def compute_signals(self, pricing_data_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for symbol_str in self.signal_progress(
            symbol_list,
            desc_str="dv2 signal precompute",
...
if __name__ == "__main__":
    ...
    strategy = DV2VixFilterStrategy(
        name="strategy_mr_dv2_vix_filter",
