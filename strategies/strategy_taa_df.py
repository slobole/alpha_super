...
Execution philosophy at rebalance open t:
    ...
    q^{intent}_{i,t} ~= floor(V^{budget}_t * w_{i,t} / P^{fill}_{i,t})
...
class DefenseFirstStrategy(Strategy):
    ...
    pre-open OPG-style rebalance, not exact open-aware account-value sizing.
    ...
    exchange for simpler live alignment. Exact open-aware rebalancing is not
    the chosen model here.
    """
    ...
    def iterate(self, data: pd.DataFrame, close: pd.DataFrame, open_prices: pd.Series):
        ...
        for asset_str in self.tradeable_asset_list:
            ...
            # Avoid zero-share no-op orders, which would otherwise create
            # artificial transactions and commissions under the generic helper.
            target_share_int = int(self.total_value * target_weight_float / open_price_float)
