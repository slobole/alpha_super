"""
QPI short-only mean-reversion strategy with IBS entry and IBS / RSI2 cover rules.
...
-----------
    ...
    cover_{i,t} = 1[IBS_{i,t} < 0.10] or 1[RSI2_{i,t} < 10]

Execution philosophy
--------------------
Equal-slot sizing with next-open execution:

    short_notional_per_trade_t = V_{t-1} / N_max
...
class QPIIbsRsiExitShortStrategy(Strategy):
    """
    QPI stock pod with overbought short entries and oversold cover rules.
    ...
    def __init__(
        ...
        min_entry_ibs_float: float = 0.90,
        exit_ibs_threshold_float: float = 0.10,
        rsi_window_int: int = 2,
        exit_rsi2_threshold_float: float = 10.0,
    ):
        super().__init__(
            name=name,
        ...
        if not 0.0 <= min_entry_ibs_float <= 1.0:
            raise ValueError("min_entry_ibs_float must lie in [0, 1].")
        ...
        self.min_entry_ibs_float = min_entry_ibs_float
    ...
    def iterate(self, data_df: pd.DataFrame, close_row_ser: pd.Series, open_price_ser: pd.Series):
        ...
        for symbol_str in short_position_ser.index:
            ...
            if cover_for_ibs_bool or cover_for_rsi2_bool:
    ...
    def get_opportunity_list(self, close_row_ser: pd.Series) -> list[str]:
        ...
        candidate_df = candidate_df[
            candidate_df["ibs_value_ser"].astype(float) > self.min_entry_ibs_float
        ]
...
if __name__ == "__main__":
    ...
    strategy = QPIIbsRsiExitShortStrategy(
        name="strategy_mr_qpi_ibs_rsi_exit_short",
