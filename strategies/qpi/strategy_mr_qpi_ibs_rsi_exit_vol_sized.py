...
ENTRY_SIZING_MODE_INVERSE_VOL_STR = "inverse_vol"
ENTRY_SIZING_MODE_DIRECT_VOL_STR = "direct_vol"
VALID_ENTRY_SIZING_MODE_SET = {
    ENTRY_SIZING_MODE_INVERSE_VOL_STR,
    ENTRY_SIZING_MODE_DIRECT_VOL_STR,
}
DEFAULT_VOLATILITY_LOOKBACK_INT = 63
...
class QPIIbsRsiExitVolSizedStrategy(QPIIbsRsiExitStrategy):
    """
    ...
    def __init__(
        ...
        entry_sizing_mode_str: str = ENTRY_SIZING_MODE_INVERSE_VOL_STR,
        volatility_lookback_int: int = DEFAULT_VOLATILITY_LOOKBACK_INT,
    ):
        super().__init__(
            name=name,
        ...
        if volatility_lookback_int < 2:
            raise ValueError("volatility_lookback_int must be at least 2.")
        self.entry_sizing_mode_str = clean_entry_sizing_mode_str(entry_sizing_mode_str)
        self.volatility_lookback_int = int(volatility_lookback_int)
        self.volatility_field_str = f"volatility_{self.volatility_lookback_int}_ser"
    ...
    def compute_signals(self, pricing_data_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # *** CRITICAL*** Entry sizing volatility must use trailing closes only.
        # The engine consumes this value at previous_bar close and fills orders
        # at the next open; future closes here would be lookahead.
        close_return_df = close_price_df.pct_change(fill_method=None)
        volatility_df = (
            close_return_df.rolling(
                window=self.volatility_lookback_int,
                min_periods=self.volatility_lookback_int,
            ).std(ddof=1)
            * np.sqrt(252.0)
        )

        volatility_feature_df = volatility_df.copy()
        volatility_feature_df.columns = pd.MultiIndex.from_tuples(
            [(symbol_str, self.volatility_field_str) for symbol_str in volatility_feature_df.columns]
        )
        return pd.concat([signal_data_df, volatility_feature_df], axis=1)
    ...
    def iterate(self, data_df: pd.DataFrame, close_row_ser: pd.Series, open_price_ser: pd.Series):
        ...
        while long_slots_int > 0 and len(opportunity_symbol_list) > 0:
            ...
            volatility_float = float(close_row_ser.get((symbol_str, self.volatility_field_str), np.nan))
            if not np.isfinite(volatility_float) or volatility_float <= 0.0:
                continue

            selected_symbol_list.append(symbol_str)
            selected_volatility_list.append(volatility_float)
            long_slots_int -= 1

        if len(selected_symbol_list) == 0:
            return

        volatility_ser = pd.Series(selected_volatility_list, index=selected_symbol_list, dtype=float)
        if self.entry_sizing_mode_str == ENTRY_SIZING_MODE_INVERSE_VOL_STR:
            raw_weight_ser = 1.0 / volatility_ser
        else:
            raw_weight_ser = volatility_ser
...
def run_variant(
    ...
    strategy_obj = QPIIbsRsiExitVolSizedStrategy(
        name=f"strategy_mr_qpi_ibs_rsi_exit_vol_sized_{clean_mode_str}",
    ...
    # volatility features, but execute only on the configured backtest calendar.
