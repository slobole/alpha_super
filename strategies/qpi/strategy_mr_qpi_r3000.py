"""
QPI long-only mean-reversion strategy on the Russell 3000 point-in-time universe.

Core formulas
-------------
For stock i on decision date t:

    r^{(1)}_{i,t} = C_{i,t} / C_{i,t-1} - 1
...
class QPIR3000Strategy(Strategy):
    """
    QPI stock pod on Russell 3000 constituents with the same rules as strategy_mr_qpi.
    ...
    Names are ranked by Turnover descending.
    """
    ...
    def compute_signals(self, pricing_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for symbol_str in symbol_idx:
            ...
            # *** CRITICAL*** This backward return must use only trailing closes.
            feature_series_map[(symbol_str, "bwd_return_ser")] = close_ser.pct_change(fill_method=None)
            # *** CRITICAL*** This 3-day return must use only trailing closes.
            feature_series_map[(symbol_str, "three_day_bwd_return_ser")] = close_ser.pct_change(
                periods=3,
                fill_method=None,
            )
            # *** CRITICAL*** QPI must be computed from trailing close history only.
            feature_series_map[(symbol_str, "qpi_value_ser")] = qp_indicator(
                close_ser,
                window_int=3,
                lookback_years_int=5,
            )
            # *** CRITICAL*** SMA200 must remain a trailing average.
            feature_series_map[(symbol_str, "sma_200_price_ser")] = close_ser.rolling(window=200).mean()
    ...
    def get_opportunity_list(self, close_row_ser: pd.Series) -> list[str]:
        ...
        candidate_df = candidate_df[
            (candidate_df["qpi_value_ser"] < 15)
            & (candidate_df["Close"] > candidate_df["sma_200_price_ser"])
            & (candidate_df["three_day_bwd_return_ser"] < 0)
...
if __name__ == "__main__":
    ...
    strategy = QPIR3000Strategy(
        name="strategy_mr_qpi_r3000",
