"""
QPI long-only mean-reversion strategy with NATR-based ranking.
...
For stock i on decision date t:
    ...
    ATR^{(14)}_{i,t} = WilderAverage(TR_{i,t}, 14)

    NATR^{(14)}_{i,t} = 100 * ATR^{(14)}_{i,t} / C_{i,t}
...
class QPINatrStrategy(Strategy):
    """
    QPI stock pod with NATR-based ranking.
    ...
    def compute_signals(self, pricing_data_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # *** CRITICAL*** The 3-day return must use only trailing close history.
        three_day_return_df = close_price_df.pct_change(
    ...
    def iterate(
        ...
        for symbol_str in long_position_ser.index:
            ...
            if len(data_df.index) < 2 or pd.isna(close_price_float):
                continue

            # *** CRITICAL*** Use the prior completed bar high only. `iloc[-2]`
            # is intentional to avoid same-bar leakage in the exit rule.
...
if __name__ == "__main__":
    ...
    strategy = QPINatrStrategy(
        name="strategy_mr_qpi_natr",
