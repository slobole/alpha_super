"""
TLT / SPY end-of-month relative-strength variant.

TL;DR: This research strategy has two monthly legs built from the first 15
trading days of each month:
1. A late-month reversal leg.
2. An early-next-month SPY-vs-TLT pair leg.
...
Late-month reversal leg in month m:
    ...
    if rel_m < 0:
        long SPY from Open_{d_{m,N_m-H+1}} to Close_{d_{m,N_m}}

Early-next-month pair leg in month m+1:

    if rel_m > 0:
        long  SPY weight +w_pair
...
DEFAULT_CONFIG = EomTltSpyVariantConfig()
...
def load_pricing_data_df(
    ...
    symbol_list = list(dict.fromkeys(list(trade_symbol_list) + list(benchmark_list)))
...
def build_month_signal_df(
    ...
    config: EomTltSpyVariantConfig = DEFAULT_CONFIG,
) -> pd.DataFrame:
...
class EomTltSpyVariantResearchStrategy(Strategy):
    """
    Research-only container for the TLT / SPY monthly variant.
    """
...
def run_variant_research_backtest(
    strategy: EomTltSpyVariantResearchStrategy,
...
if __name__ == "__main__":
    ...
    open_price_df = pricing_data_df.xs("Open", axis=1, level=1)[list(config.trade_symbol_list)].astype(float)
    ...
    strategy = EomTltSpyVariantResearchStrategy(
        name="strategy_eom_tlt_spy_variant_research",
