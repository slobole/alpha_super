"""
ZROZ / SPY signal, SSO execution, L2-vol-normalized end-of-month variant.

TL;DR: This research strategy mirrors `strategy_eom_zroz_spy_sso_variant.py`,
but compares standard-deviation-normalized log returns instead of raw simple
returns.
...
Define:
    ...
    W = 63   # trailing L2 volatility window
...
Trailing standard-deviation daily volatility estimate:

    l2_vol_asset_m
        = std(log(Close_asset_t / Close_asset_{t-1}), W)

where the standard deviation uses the W daily returns ending before d_{m,1}.
...
Normalized scores:
    ...
    score_spy_m  = g_spy_m^{(15)}  / (l2_vol_SPY_m  * sqrt(L))
    score_zroz_m = g_zroz_m^{(15)} / (l2_vol_ZROZ_m * sqrt(L))
...
class EomZrozSpySsoL2NormVariantConfig(EomZrozSpySsoVariantConfig):
    vol_window_day_count_int: int = 63
    vol_window_mode_str: str = "trailing_63"
    min_l2_daily_vol_float: float = 1e-8

    def __post_init__(self):
        super().__post_init__()
        if self.vol_window_day_count_int <= 1:
            raise ValueError("vol_window_day_count_int must be greater than 1.")
        ...
        if self.min_l2_daily_vol_float <= 0.0:
            raise ValueError("min_l2_daily_vol_float must be positive.")


DEFAULT_CONFIG = EomZrozSpySsoL2NormVariantConfig()


class EomZrozSpySsoL2NormVariantResearchStrategy(EomZrozSpySsoVariantResearchStrategy):
    """
    Research-only container for the L2-normalized ZROZ / SPY-signal / SSO-execution variant.
    """
...
def build_month_signal_df(
    ...
    config: EomZrozSpySsoL2NormVariantConfig = DEFAULT_CONFIG,
) -> pd.DataFrame:
    ...
    for month_period, month_date_index in month_group_map.items():
        ...
        spy_l2_daily_vol_float = float(vol_window_log_return_df[config.signal_symbol_str].std(ddof=1))
        zroz_l2_daily_vol_float = float(vol_window_log_return_df[config.defense_symbol_str].std(ddof=1))
        if (
            spy_l2_daily_vol_float <= config.min_l2_daily_vol_float
            or zroz_l2_daily_vol_float <= config.min_l2_daily_vol_float
        ):
            continue
        ...
        spy_l2_score_float = float(
            spy_first15_log_return_float / (spy_l2_daily_vol_float * signal_horizon_scale_float)
        )
        zroz_l2_score_float = float(
            zroz_first15_log_return_float / (zroz_l2_daily_vol_float * signal_horizon_scale_float)
        )
        rel_l2_score_float = float(spy_l2_score_float - zroz_l2_score_float)
        spy_outperformed_bool = bool(rel_l2_score_float > 0.0)
        zroz_outperformed_bool = bool(rel_l2_score_float < 0.0)
        ...
        month_signal_row_list.append(
            {
                ...
                "spy_l2_daily_vol_float": spy_l2_daily_vol_float,
                "zroz_l2_daily_vol_float": zroz_l2_daily_vol_float,
                "vol_window_mode_str": config.vol_window_mode_str,
                "spy_l2_score_float": spy_l2_score_float,
                "zroz_l2_score_float": zroz_l2_score_float,
                "rel_l2_score_float": rel_l2_score_float,
    ...
    if len(month_signal_df) == 0:
        return pd.DataFrame(
            columns=[
                ...
                "spy_l2_daily_vol_float",
                "zroz_l2_daily_vol_float",
                "vol_window_mode_str",
                "spy_l2_score_float",
                "zroz_l2_score_float",
                "rel_l2_score_float",
            ...
            trade_leg_row_list.append(
                {
                    ...
                    "rel_l2_score_float": float(signal_row_ser["rel_l2_score_float"]),
                }
            )
...
if __name__ == "__main__":
    ...
    strategy = EomZrozSpySsoL2NormVariantResearchStrategy(
        name="strategy_eom_zroz_spy_sso_l2_norm_variant_research",
