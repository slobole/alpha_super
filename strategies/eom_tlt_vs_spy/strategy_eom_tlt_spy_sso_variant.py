"""
TLT / SPY signal, SSO execution, end-of-month relative-strength variant.

TL;DR: This research strategy keeps the monthly signal comparison on `SPY`
versus `TLT`, but replaces every long `SPY` expression with long `SSO`.
...
-------------------------
Because the request did not specify leverage, the pair leg uses:
...
Causality guards
----------------
To avoid signal / entry overlap in the reversal leg, month m must satisfy:

    N_m >= L + H

Otherwise month m is skipped for the reversal trade.

The pair leg only requires:

    N_{m+1} >= K

because its signal comes from the prior month.
...
class EomTltSpySsoVariantConfig:
    signal_symbol_str: str = "SPY"
    defense_symbol_str: str = "TLT"
    long_symbol_str: str = "SSO"
    benchmark_list: tuple[str, ...] = ("SPY", "SSO")
    ...
    def __post_init__(self):
        symbol_str_list = [self.signal_symbol_str, self.defense_symbol_str, self.long_symbol_str]
        if any(len(symbol_str) == 0 for symbol_str in symbol_str_list):
            raise ValueError("signal_symbol_str, defense_symbol_str, and long_symbol_str must be non-empty.")
        if self.signal_symbol_str == self.defense_symbol_str:
            raise ValueError("signal_symbol_str and defense_symbol_str must differ.")
        if self.long_symbol_str == self.defense_symbol_str:
            raise ValueError("long_symbol_str and defense_symbol_str must differ.")
    ...
    def data_symbol_list(self) -> tuple[str, ...]:
        return tuple(
            dict.fromkeys(
                [self.signal_symbol_str, self.defense_symbol_str, self.long_symbol_str, *self.benchmark_list]
            )
        )
...
DEFAULT_CONFIG = EomTltSpySsoVariantConfig()


def load_pricing_data_df(
    config: EomTltSpySsoVariantConfig = DEFAULT_CONFIG,
) -> pd.DataFrame:
...
class EomTltSpySsoVariantResearchStrategy(Strategy):
    """
    Research-only container for the TLT / SPY-signal / SSO-execution variant.
    """
...
def run_variant_research_backtest(
    strategy: EomTltSpySsoVariantResearchStrategy,
...
if __name__ == "__main__":
    ...
    strategy = EomTltSpySsoVariantResearchStrategy(
        name="strategy_eom_tlt_spy_sso_variant_research",
