"""
TLT / SPY signal, VIX-gated SPY/UPRO execution, end-of-month relative-strength variant.

TL;DR: Keep the original monthly `SPY` versus `TLT` signal. Whenever the
strategy would otherwise go long `SPY`, inspect the latest prior-close VIX
level. If that VIX close is below 15, execute with `UPRO`; otherwise, execute
with `SPY`.
...
Equity execution symbol:

    equity_asset_e
        = UPRO, if vix_e < theta_vix
        = SPY,  otherwise

Late-month reversal leg in month m:

    if rel_m > 0:
        long TLT from Open_{d_{m,N_m-H+1}} to Close_{d_{m,N_m}}

    if rel_m < 0:
        long equity_asset_{eom,m}
        from Open_{d_{m,N_m-H+1}} to Close_{d_{m,N_m}}

Early-next-month pair leg in month m+1:

    if rel_m > 0:
        long  equity_asset_{bom,m+1} weight +w_pair
        short TLT                  weight -w_pair
        from Open_{d_{m+1,1}} to Close_{d_{m+1,K}}

Data-source note
----------------
The user requested a `VIX.C` threshold check. This research file uses Yahoo
data, so the VIX level proxy is Yahoo's `^VIX` close series.
...
class EomTltSpyUproVixVariantConfig:
    signal_symbol_str: str = "SPY"
    defense_symbol_str: str = "TLT"
    standard_long_symbol_str: str = "SPY"
    leveraged_long_symbol_str: str = "UPRO"
    ...
    vix_threshold_float: float = 15.0
    ...
    def __post_init__(self):
        symbol_str_list = [
            ...
            self.standard_long_symbol_str,
            self.leveraged_long_symbol_str,
            self.vix_symbol_str,
        ]
        if any(len(symbol_str) == 0 for symbol_str in symbol_str_list):
            raise ValueError("All symbol strings must be non-empty.")
        if self.signal_symbol_str != self.standard_long_symbol_str:
            raise ValueError("signal_symbol_str must match standard_long_symbol_str for this variant.")
        if self.defense_symbol_str in {self.standard_long_symbol_str, self.leveraged_long_symbol_str}:
            raise ValueError("defense_symbol_str must differ from the long-side execution symbols.")
        if self.vix_symbol_str in {self.defense_symbol_str, self.standard_long_symbol_str, self.leveraged_long_symbol_str}:
            raise ValueError("vix_symbol_str must differ from the tradeable symbols.")
        ...
        if not np.isfinite(self.vix_threshold_float) or self.vix_threshold_float <= 0.0:
            raise ValueError("vix_threshold_float must be positive and finite.")
...
DEFAULT_CONFIG = EomTltSpyUproVixVariantConfig()


def load_pricing_data_df(
    config: EomTltSpyUproVixVariantConfig = DEFAULT_CONFIG,
) -> pd.DataFrame:
...
def choose_equity_execution_symbol_str(
    vix_close_float: float,
    config: EomTltSpyUproVixVariantConfig = DEFAULT_CONFIG,
) -> str:
    if np.isfinite(vix_close_float) and vix_close_float < float(config.vix_threshold_float):
        return config.leveraged_long_symbol_str
    return config.standard_long_symbol_str
...
def build_month_signal_df(
    ...
    for month_period, month_date_index in month_group_map.items():
        ...
        reversal_asset_str: str | None = None
        ...
        if (
            ...
            if reversal_entry_bar_ts > signal_end_bar_ts:
                if spy_outperformed_bool:
                    ...
                    # *** CRITICAL*** The VIX gate must use the latest close
                    ...
                    reversal_asset_str = choose_equity_execution_symbol_str(
        ...
        # month's completed first-15-day signal. The VIX gate uses the last
        ...
        if spy_outperformed_bool and next_month_period in month_group_map:
            ...
            if len(next_month_trading_index) >= config.bom_pair_hold_day_count_int:
                ...
                pair_long_asset_str = choose_equity_execution_symbol_str(
...
def build_trade_leg_plan_df(
    ...
    for _, signal_row_ser in month_signal_df.iterrows():
        ...
        if pd.notna(pair_entry_bar_ts) and pd.notna(pair_exit_bar_ts) and pair_long_asset_str is not None:
            trade_id_int += 1
            trade_leg_row_list.append(
                {
                    "trade_id_int": trade_id_int,
                    "leg_type_str": "pair_long_equity",
                    "signal_month_period_str": signal_row_ser.name,
...
class EomTltSpyUproVixVariantResearchStrategy(Strategy):
    """
    Research-only container for the TLT / SPY signal / VIX-gated SPY-UPRO execution variant.
    """
...
def run_variant_research_backtest(
    strategy: EomTltSpyUproVixVariantResearchStrategy,
    pricing_data_df: pd.DataFrame,
) -> EomTltSpyUproVixVariantResearchStrategy:
...
if __name__ == "__main__":
    ...
    strategy = EomTltSpyUproVixVariantResearchStrategy(
        name="strategy_eom_tlt_spy_upro_vix_variant_research",
