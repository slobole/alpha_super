"""
Run the Defense First fallback VIX-cash variant grid and save a summary table.
    ...
    for base_strategy_name_str in BASE_STRATEGY_NAME_TUPLE:
        for fallback_asset_str in REQUESTED_FALLBACK_ASSET_TUPLE:
            ...
            module_name_list.append(
                f"{base_strategy_name_str}_fallback_{fallback_key_str}_vix_cash"
            )
...
def run_variant_suite(
    ...
    if len(variant_metric_df) != len(module_name_tuple):
        raise AssertionError(
            f"Expected {len(module_name_tuple)} VIX-cash variants, found {len(variant_metric_df)}."
        )

    timestamp_str = datetime.now().strftime("%Y-%m-%d_%H%M%S")
    output_dir_path = Path(output_dir_str) / "taa_df_fallback_vix_cash_variant_suite" / timestamp_str
