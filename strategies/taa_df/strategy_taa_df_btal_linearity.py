...
rank-weighted slots, and a simple regression-based linearity signal.
...
Quantitative consequence versus the annualized Clenow rank variant:
1. Old behavior: score = R^2 * annualized log slope.
2. New behavior: score = adjusted R^2 * raw daily log slope.
...
linearity_lookback_day_vec = (21, 63, 126, 252)
linearity_signal_threshold_float = 0.0
...
def compute_linearity_signal_from_log_price_window_vec(
    log_price_window_vec: np.ndarray,
    x_centered_vec: np.ndarray,
    sxx_float: float,
) -> float:
    """
    Compute a simple linearity signal from one trailing log-price window.
    ...
    return adjusted_r_squared_float * slope_float


def compute_daily_linearity_score_df(
    signal_close_df: pd.DataFrame,
    lookback_day_vec: Sequence[int] = linearity_lookback_day_vec,
) -> pd.DataFrame:
    """
    Compute daily composite linearity scores from TOTALRETURN closes.
    """
    ...
    for lookback_day_int in lookback_day_vec:
        ...
        for asset_str in log_signal_close_df.columns:
            ...
            asset_score_ser = log_price_ser.rolling(lookback_day_int).apply(
                lambda log_price_window_vec: compute_linearity_signal_from_log_price_window_vec(
    ...
    daily_linearity_score_df = sum(lookback_score_df_list) / float(len(lookback_score_df_list))
    return daily_linearity_score_df


def compute_month_end_rank_weight_df_from_daily_linearity_score_df(
    daily_linearity_score_df: pd.DataFrame,
    config: DefenseFirstConfig,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Sample daily linearity scores at month-end and build rank-weighted targets.
    """
...
def get_defense_first_linearity_rank_data(
    ...
    month_end_score_df, month_end_weight_df = compute_month_end_rank_weight_df_from_daily_linearity_score_df(
...
if __name__ == "__main__":
    ...
    (
        ...
        month_end_weight_df,
        rebalance_weight_df,
    ) = get_defense_first_linearity_rank_data(taa_config)

    strategy = DefenseFirstStrategy(
        name="strategy_taa_df_btal_linearity",
