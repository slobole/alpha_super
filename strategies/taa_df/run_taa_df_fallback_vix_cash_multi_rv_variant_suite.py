"""
Run the Defense First fallback multi-lookback VIX-cash variant grid and save a summary table.
...
def _variant_module_name_tuple() -> tuple[str, ...]:
    ...
    for base_strategy_name_str in BASE_STRATEGY_NAME_TUPLE:
        for fallback_asset_str in REQUESTED_FALLBACK_ASSET_TUPLE:
            fallback_key_str = fallback_asset_str.lower()
            module_name_list.append(
                f"{base_strategy_name_str}_fallback_{fallback_key_str}_vix_cash_multi_rv"
            )
    return tuple(module_name_list)
...
def run_variant_suite(
    ...
    module_name_tuple = _variant_module_name_tuple()

    for module_name_str in module_name_tuple:
    ...
    if len(variant_metric_df) != len(module_name_tuple):
        raise AssertionError(
            f"Expected {len(module_name_tuple)} multi-RV VIX-cash variants, found {len(variant_metric_df)}."
        )

    timestamp_str = datetime.now().strftime("%Y-%m-%d_%H%M%S")
    output_dir_path = Path(output_dir_str) / "taa_df_fallback_vix_cash_multi_rv_variant_suite" / timestamp_str
