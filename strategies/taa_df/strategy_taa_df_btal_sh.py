"""
Defense First tactical allocation with BTAL and SH added to the defensive
sleeve and SPY as the fallback sleeve.

This variant preserves the exact signal and execution logic from
`strategy_taa_df.py` and changes only the configuration:

    defensive_asset_list = ("GLD", "UUP", "TLT", "DBC", "BTAL", "SH")
    fallback_asset = "SPY"

The defensive rank weights switch from fixed four-slot weights to six-slot
rank weights:

    rank_score_vec = [6, 5, 4, 3, 2, 1]
...
Quantitative consequence:
    ...
    start_date_str = max(first_BTAL_date, first_SH_date, first_SPY_date)
                   = max(2011-09-13, 2006-06-21, 1993-01-29)
                   = 2011-09-13

Because BTAL starts later than SH and SPY, the evaluation window is
...
sh_inception_date_str = "2006-06-21"
spy_inception_date_str = "1993-01-29"
inception_date_str_tuple = (
    btal_inception_date_str,
    sh_inception_date_str,
    spy_inception_date_str,
)
btal_sh_effective_start_date_str = max(inception_date_str_tuple)
btal_sh_defensive_asset_list = ("GLD", "UUP", "TLT", "DBC", "BTAL", "SH")
btal_sh_rank_weight_vec = (
    6.0 / 21.0,
    5.0 / 21.0,
    4.0 / 21.0,
    3.0 / 21.0,
...
DEFAULT_CONFIG = DefenseFirstConfig(
    defensive_asset_list=btal_sh_defensive_asset_list,
    fallback_asset="SPY",
    rank_weight_vec=btal_sh_rank_weight_vec,
    start_date_str=btal_sh_effective_start_date_str,
)
