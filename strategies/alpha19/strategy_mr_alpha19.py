"""
Alpha 19 long-only pullback strategy for point-in-time S&P 500 members.
...
reduces to the same sign term because:

    close_t - close_{t-7} + (close_t - close_{t-7})
        = 2 * (close_t - close_{t-7})
...
class Alpha19PullbackStrategy(Strategy):
    """
    Long-only Alpha 19 stock pod for buying short-term pullbacks in winners.
    ...
    Exit rules:
        ...
        exit_{i,t} = 1[holding_days_{i,t} >= 5] or 1[alpha19_{i,t} <= 0]
    ...
    def __init__(
        ...
        max_holding_days_int: int = 12,
        max_entry_ibs_float: float = 0.2,
    ):
        super().__init__(
            name=name,
    ...
    def iterate(self, data: pd.DataFrame, close: pd.Series, open_prices: pd.Series):
        ...
        for symbol_str in long_position_ser.index:
            alpha19_signal_float = close.get((symbol_str, "alpha19_signal_ser"), np.nan)
            exit_for_signal_bool = pd.notna(alpha19_signal_float) and float(alpha19_signal_float) <= 0.0

            entry_transaction_ser = self.get_latest_transaction(symbol_str)
            entry_bar_ts = pd.Timestamp(entry_transaction_ser["bar"])
            exit_for_holding_period_bool = False

            if entry_bar_ts in data.index and self.previous_bar in data.index:
                ...
                exit_for_holding_period_bool = holding_days_int >= self.max_holding_days_int

            if exit_for_signal_bool or exit_for_holding_period_bool:
...
if __name__ == "__main__":
    ...
    strategy = Alpha19PullbackStrategy(
        name="strategy_mr_alpha19",
