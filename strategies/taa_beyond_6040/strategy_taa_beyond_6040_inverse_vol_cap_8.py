"""
Beyond 60/40 inverse-volatility strategy with an 8% volatility target.

Base weights are the same as the inverse-volatility strategy:

    w_{i,t}^{base}
        = (1 / sigma_{i,t}^{(63)}) / sum_j(1 / sigma_{j,t}^{(63)})

The risky-asset exposure is scaled down only when trailing realized portfolio
volatility is above 8.5%:
...
There is no leverage. If the estimated portfolio volatility is below the
...
STRATEGY_NAME_STR = "strategy_taa_beyond_6040_inverse_vol_cap_8"
...
def run_variant(
    ...
    # and volatility-cap signal formation; only the executable fill calendar is clipped.
