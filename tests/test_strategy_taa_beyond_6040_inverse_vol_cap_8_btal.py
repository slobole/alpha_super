...
from strategies.taa_beyond_6040 import strategy_taa_beyond_6040_inverse_vol_cap_8_btal as variant_module


MODULE_IMPORT_STR = "strategies.taa_beyond_6040.strategy_taa_beyond_6040_inverse_vol_cap_8_btal"
...
def make_pricing_data_df(num_days_int: int = 220) -> pd.DataFrame:
    ...
    return_vec_by_symbol_dict = {
        ...
        "BTAL": 0.0001 + 0.0070 * np.sin(bar_idx_vec / 5.0 + 1.2),
...
def test_default_config_adds_btal_with_8pct_target_85pct_trigger():
    assert variant_module.DEFAULT_CONFIG.asset_list == ("VTI", "GLD", "TLT", "BTAL")
...
def test_run_variant_honors_bench_run_contract_with_btal():
    ...
    assert captured_config_list[0].asset_list == ("VTI", "GLD", "TLT", "BTAL")
    assert captured_config_list[0].end_date_str == "2023-09-01"
    assert strategy_obj.name == variant_module.STRATEGY_NAME_STR
    assert strategy_obj.asset_list == ["VTI", "GLD", "TLT", "BTAL"]
    assert strategy_obj._capital_base == 12_345.0
    assert {"VTI", "GLD", "TLT", "BTAL", "Cash"}.issubset(strategy_obj.daily_target_weights.columns)
