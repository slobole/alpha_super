...
from strategies.eom_tlt_vs_spy.strategy_eom_tlt_spy_sso_ubt_variant import (
    EomTltSpySsoUbtVariantConfig,
    EomTltSpySsoUbtVariantResearchStrategy,
...
class EomTltSpySsoUbtVariantTests(unittest.TestCase):
    @staticmethod
    ...
    def make_open_close_df(
        ...
        ubt_close_list: list[float] | None = None,
        open_price_float: float = 100.0,
    ) -> tuple[pd.DataFrame, pd.DataFrame]:
        ubt_close_list = tlt_close_list if ubt_close_list is None else ubt_close_list
        open_price_df = pd.DataFrame(
            {
                ...
                "UBT": pd.Series(open_price_float, index=trading_index, dtype=float),
            }
        )
        close_price_df = pd.DataFrame(
            {
                ...
                "UBT": pd.Series(ubt_close_list, index=trading_index, dtype=float),
            }
        )
    ...
    def make_strategy(
        ...
        return EomTltSpySsoUbtVariantResearchStrategy(
            name="EomTltSpySsoUbtVariantTest",
            benchmarks=[],
            tradeable_asset_list=["SSO", "UBT"],
    ...
    def test_build_month_signal_df_uses_tlt_signal_but_sets_ubt_reversal_and_pair(self):
        ...
        ubt_close_list = [100.0 + idx_int for idx_int in range(1, 23)] + [100.0, 100.0, 100.0, 100.0, 100.0]
        open_price_df, close_price_df = self.make_open_close_df(
            ...
            ubt_close_list=ubt_close_list,
        )

        config = EomTltSpySsoUbtVariantConfig()
        ...
        self.assertEqual(config.data_symbol_list, ("SPY", "TLT", "SSO", "UBT"))
        self.assertEqual(config.tradeable_symbol_list, ("SSO", "UBT"))
        ...
        self.assertEqual(str(signal_row_ser["reversal_asset_str"]), "UBT")
        ...
        self.assertEqual(str(trade_leg_plan_df.iloc[2]["asset_str"]), "UBT")
    ...
    def test_build_daily_target_weight_df_and_backtest_cover_reversal_and_pair_legs(self):
        ...
        self.assertAlmostEqual(float(daily_target_weight_df.loc[pd.Timestamp("2024-01-30"), "UBT"]), 1.0)
        self.assertAlmostEqual(float(daily_target_weight_df.loc[pd.Timestamp("2024-02-01"), "SSO"]), 0.5)
        self.assertAlmostEqual(float(daily_target_weight_df.loc[pd.Timestamp("2024-02-01"), "UBT"]), -0.5)
        self.assertAlmostEqual(float(daily_target_weight_df.loc[pd.Timestamp("2024-02-08"), "SSO"]), 0.0)
        self.assertAlmostEqual(float(daily_target_weight_df.loc[pd.Timestamp("2024-02-08"), "UBT"]), 0.0)
        ...
        pricing_data_df = pd.DataFrame(
            {
                ...
                ("UBT", "Open"): pd.Series(100.0, index=trading_index, dtype=float),
                ("UBT", "High"): pd.Series([101.0, 111.0, 101.0, 99.0, 97.0, 95.0, 91.0, 91.0], index=trading_index, dtype=float),
                ("UBT", "Low"): pd.Series([99.0, 109.0, 99.0, 97.0, 95.0, 93.0, 89.0, 89.0], index=trading_index, dtype=float),
                ("UBT", "Close"): pd.Series([100.0, 110.0, 100.0, 98.0, 96.0, 94.0, 90.0, 90.0], index=trading_index, dtype=float),
            },
            index=trading_index,
        )
