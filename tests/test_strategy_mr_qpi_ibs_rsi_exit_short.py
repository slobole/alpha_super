...
class QPIIbsRsiExitShortStrategyTests(unittest.TestCase):
    def make_strategy(self, **kwargs) -> QPIIbsRsiExitShortStrategy:
        base_kwargs = dict(
            name="QPIIbsRsiExitShortTest",
            benchmarks=[],
        ...
        return QPIIbsRsiExitShortStrategy(**base_kwargs)
    ...
    def test_compute_signals_adds_expected_features_and_rsi2_matches_talib(self):
    ...
    def test_get_opportunity_list_enforces_mirrored_short_filters_and_ranking(self):
        strategy = self.make_strategy()
    ...
    def test_iterate_submits_cover_order_when_ibs_crosses_threshold(self):
        strategy = self.make_strategy()
        strategy.previous_bar = pd.Timestamp("2024-03-08")
    ...
    def test_iterate_submits_cover_order_when_rsi2_crosses_threshold(self):
        strategy = self.make_strategy()
        strategy.previous_bar = pd.Timestamp("2024-03-08")
        strategy.current_bar = pd.Timestamp("2024-03-11")
        strategy.universe_df = pd.DataFrame({"AAA": [1]}, index=[strategy.previous_bar])
        ...
        close_row_ser = self.make_close_row_ser(
            {
                ...
                ("BBB", "ibs_value_ser"): 0.97,
                ("BBB", "rsi2_value_ser"): 80.0,
                ("CCC", "Close"): 93.0,
                ("CCC", "Turnover"): 40_000_000.0,
                ("CCC", "qpi_value_ser"): 9.0,
                ("CCC", "sma_200_price_ser"): 100.0,
                ("CCC", "three_day_return_ser"): 0.03,
                ("CCC", "ibs_value_ser"): 0.95,
                ("CCC", "rsi2_value_ser"): 70.0,
            }
        )
