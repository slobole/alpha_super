...
from strategies.eom_tlt_vs_spy.strategy_eom_tlt_spy_vix_size_variant import (
    DEFAULT_CONFIG,
    build_month_signal_df,
    build_trade_leg_plan_df,
    compute_spy_weight_scale_float,
)


class EomTltSpyVixSizeVariantTests(unittest.TestCase):
    @staticmethod
    ...
    def test_compute_spy_weight_scale_float_clips_inverse_vix_rule(self):
        self.assertAlmostEqual(compute_spy_weight_scale_float(10.0, DEFAULT_CONFIG), 1.5)
        self.assertAlmostEqual(compute_spy_weight_scale_float(20.0, DEFAULT_CONFIG), 1.0)
        self.assertAlmostEqual(compute_spy_weight_scale_float(40.0, DEFAULT_CONFIG), 0.5)
        self.assertAlmostEqual(compute_spy_weight_scale_float(np.nan, DEFAULT_CONFIG), 1.0)

    def test_reversal_spy_weight_uses_prior_close_vix(self):
        ...
        self.assertAlmostEqual(float(january_signal_row_ser["reversal_spy_weight_scale_float"]), 1.5)
        self.assertAlmostEqual(float(january_signal_row_ser["reversal_signed_weight_float"]), 1.5)

    def test_pair_spy_weight_scales_only_spy_leg(self):
        ...
        january_signal_row_ser = month_signal_df.loc["2024-01"]
        self.assertAlmostEqual(float(january_signal_row_ser["pair_spy_weight_scale_float"]), 0.5)
        self.assertAlmostEqual(float(january_signal_row_ser["pair_long_signed_weight_float"]), 0.25)

        pair_long_spy_row_ser = trade_leg_plan_df[trade_leg_plan_df["leg_type_str"] == "pair_long_spy"].iloc[0]
        pair_short_tlt_row_ser = trade_leg_plan_df[trade_leg_plan_df["leg_type_str"] == "pair_short_tlt"].iloc[0]
        self.assertAlmostEqual(float(pair_long_spy_row_ser["signed_weight_float"]), 0.25)
