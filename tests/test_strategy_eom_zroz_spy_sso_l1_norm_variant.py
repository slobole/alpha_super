...
from strategies.eom_tlt_vs_spy.strategy_eom_zroz_spy_sso_l1_norm_variant import (
    EomZrozSpySsoL1NormVariantConfig,
    EomZrozSpySsoL1NormVariantResearchStrategy,
...
class EomZrozSpySsoL1NormVariantTests(unittest.TestCase):
    @staticmethod
    def make_l1_index() -> pd.DatetimeIndex:
    ...
    def make_open_close_df(
        ...
        trading_index = self.make_l1_index()
    ...
    def make_strategy(
        ...
        return EomZrozSpySsoL1NormVariantResearchStrategy(
            name="EomZrozSpySsoL1NormVariantTest",
            benchmarks=[],
    ...
    def test_build_month_signal_df_uses_l1_normalized_log_returns_and_sets_zroz_pair(self):
        ...
        config = EomZrozSpySsoL1NormVariantConfig()
        ...
        expected_spy_l1_daily_vol_float = math.sqrt(math.pi / 2.0) * 0.010
        expected_zroz_l1_daily_vol_float = math.sqrt(math.pi / 2.0) * 0.005
        ...
        expected_spy_score_float = expected_spy_log_return_float / (
            expected_spy_l1_daily_vol_float * math.sqrt(config.signal_day_count_int)
        )
        expected_zroz_score_float = expected_zroz_log_return_float / (
            expected_zroz_l1_daily_vol_float * math.sqrt(config.signal_day_count_int)
        )
        ...
        self.assertAlmostEqual(float(signal_row_ser["spy_l1_daily_vol_float"]), expected_spy_l1_daily_vol_float)
        self.assertAlmostEqual(float(signal_row_ser["zroz_l1_daily_vol_float"]), expected_zroz_l1_daily_vol_float)
        self.assertAlmostEqual(float(signal_row_ser["spy_l1_score_float"]), expected_spy_score_float)
        self.assertAlmostEqual(float(signal_row_ser["zroz_l1_score_float"]), expected_zroz_score_float)
    ...
    def test_l1_normalization_can_flip_raw_return_winner(self):
        open_price_df, close_price_df = self.make_open_close_df(
        ...
        self.assertGreater(
            float(signal_row_ser["zroz_l1_score_float"]),
            float(signal_row_ser["spy_l1_score_float"]),
        )
    ...
    def test_month_to_date_signal_mode_uses_signal_window_returns_without_prior_history(self):
        ...
        config = EomZrozSpySsoL1NormVariantConfig(vol_window_mode_str="month_to_date_signal")
        month_signal_df = build_month_signal_df(
            open_price_df=open_price_df,
            close_price_df=close_price_df,
            config=config,
        ...
        expected_spy_log_return_float = math.log(116.0 / 100.0)
        expected_zroz_log_return_float = math.log(106.5 / 100.0)
        expected_spy_l1_daily_vol_float = math.sqrt(math.pi / 2.0) * abs(expected_spy_log_return_float) / 15.0
        expected_zroz_l1_daily_vol_float = math.sqrt(math.pi / 2.0) * abs(expected_zroz_log_return_float) / 15.0

        self.assertEqual(str(signal_row_ser["vol_window_mode_str"]), "month_to_date_signal")
        self.assertAlmostEqual(float(signal_row_ser["spy_l1_daily_vol_float"]), expected_spy_l1_daily_vol_float)
        self.assertAlmostEqual(float(signal_row_ser["zroz_l1_daily_vol_float"]), expected_zroz_l1_daily_vol_float)

    def test_build_month_signal_df_skips_when_trailing_l1_window_is_missing(self):
        trading_index = pd.bdate_range(start="2024-01-01", end="2024-02-08")
        open_price_df = pd.DataFrame(
            {
                "SPY": pd.Series(100.0, index=trading_index, dtype=float),
