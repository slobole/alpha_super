...
def make_execution_price_df(symbol_list: list[str]) -> pd.DataFrame:
    ...
    for symbol_idx_int, symbol_str in enumerate(symbol_list):
        base_price_list = [
            100.0 + float(symbol_idx_int * 5) + float(day_idx_int) * 0.1
            for day_idx_int in range(len(execution_index))
        ]
        price_df = pd.DataFrame(
            {
                "Open": base_price_list,
                "High": [price_float * 1.002 for price_float in base_price_list],
                "Low": [price_float * 0.998 for price_float in base_price_list],
                "Close": [price_float * 1.001 for price_float in base_price_list],
            },
...
def make_standard_loader_output(config) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    # *** CRITICAL*** This fixture preserves the production TAA timing:
    # month-end decision close, first tradable open of the following month.
...
class DefenseFirstStrategyTests(unittest.TestCase):
    ...
    def test_base_and_btal_modules_expose_vanilla_run_variant_contract(self):
        for module_import_str, expected_strategy_name_str in (
            ("strategies.taa_df.strategy_taa_df", "strategy_taa_df"),
            ("strategies.taa_df.strategy_taa_df_btal", "strategy_taa_df_btal"),
        ):
