...
from strategies.vix_stuff.strategy_mr_vix1d_vixy_long import (
    Vix1dVixyLongStrategy,
    compute_vix1d_vixy_long_signal_df,
)


class Vix1dVixyLongStrategyTests(unittest.TestCase):
    def make_strategy(self, **kwargs) -> Vix1dVixyLongStrategy:
        base_kwargs = dict(
            name="Vix1dVixyLongTest",
            benchmarks=["SPY"],
        ...
        return Vix1dVixyLongStrategy(**base_kwargs)
        ...
        pricing_data_df = pd.DataFrame(
            {
                ...
                ("$VIX1D", "Open"): [12.0, 9.0, 8.0, 11.0, 8.0, 12.0],
                ("$VIX1D", "High"): [12.5, 9.5, 8.5, 11.5, 8.5, 12.5],
                ("$VIX1D", "Low"): [11.5, 8.5, 7.5, 10.5, 7.5, 11.5],
                ("$VIX1D", "Close"): [12.0, 9.0, 8.0, 11.0, 8.0, 12.0],
    ...
    def test_compute_vix1d_vixy_long_signal_df_matches_threshold_and_term_filter_logic(self):
        date_index = pd.date_range("2024-01-02", periods=4, freq="B")
        vix1d_close_ser = pd.Series([12.0, 9.0, 8.0, 8.0], index=date_index, dtype=float)
        ...
        signal_df = compute_vix1d_vixy_long_signal_df(
        ...
        self.assertFalse(bool(signal_df.loc[date_index[0], "long_signal_bool"]))
        ...
        self.assertTrue(bool(signal_df.loc[date_index[1], "long_signal_bool"]))
        ...
        self.assertFalse(bool(signal_df.loc[date_index[2], "long_signal_bool"]))
        ...
        self.assertFalse(bool(signal_df.loc[date_index[3], "long_signal_bool"]))
    ...
    def test_compute_signals_adds_expected_features_and_passes_signal_audit(self):
        ...
        self.assertIn(("VIXY", "vix1d_close_ser"), signal_data_df.columns)
        self.assertIn(("VIXY", "vix_close_ser"), signal_data_df.columns)
        self.assertIn(("VIXY", "vix3m_close_ser"), signal_data_df.columns)
        self.assertIn(("VIXY", "term_spread_ser"), signal_data_df.columns)
        self.assertIn(("VIXY", "trade_filter_ok_bool"), signal_data_df.columns)
        self.assertIn(("VIXY", "long_signal_bool"), signal_data_df.columns)
        self.assertIn(("VIXY", "target_weight_ser"), signal_data_df.columns)
    ...
    def test_iterate_submits_exit_when_signal_turns_off(self):
        ...
        strategy.add_transaction(4, strategy.previous_bar, "VIXY", 100, 20.0, 2_000.0, 1, 0.0)

        close_row_ser = pd.Series(
            {
                ("VIXY", "target_weight_ser"): 0.0,
            }
        )
        close_row_ser.index = pd.MultiIndex.from_tuples(close_row_ser.index)
        open_price_ser = pd.Series({"VIXY": 20.0}, dtype=float)
