...
| G-012 | Capacity and tradability | `CapacityAnalysis` reruns supported strategies across an AUM grid, uses lagged 10-day mean and 20-day median dollar ADV, applies separate MOO/MOC rules, and reports Recommended/Outer capacity with order-level evidence. | The model still lacks real auction imbalance data, partial fills, routing, and calibrated live TCA. MOO remains constraint-only because the current inputs do not provide reliable point-in-time exchange and size buckets. | High. | Keep the normal IBKR Fixed plus 2.5 bps baseline, require explicit MOO/MOC policy, use conservative Order/ADV limits, and label unestimable MOO stress/break-even results plainly. | Calibrate the capacity model from live fills and auction data without changing historical signal or order timing semantics. | Active mitigation |
