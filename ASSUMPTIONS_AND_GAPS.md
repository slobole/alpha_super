...
| G-020 | Brokerless incubation execution | `mode: incubation` uses a local virtual broker that accepts order intent, sizes open-execution policies from prior-session Norgate Close, settles open fills from IBKR `ticker.open`, records cash ledger rows, and updates pod state without sending orders to IBKR. | This proves daily process mechanics, state persistence, same-day price-read plumbing, and accounting, but it does not prove broker order execution, broker rejects, partial fills, auction imbalance behavior, latency, or real execution quality. IBKR `ticker.open` is an operational same-day source, not the final exchange-audited Norgate EOD source. | Medium if mistaken for broker execution validation; Low when used only as the intended promotion gate before IBKR paper and tiny live. | Keep incubation routes clearly virtual via `SIM_`, use a separate price-read client ID, report slippage versus reference prices, block `same_day_moc` without a causal pre-close snapshot, and require later IBKR paper/tiny-live gates. | Add broker/live reconciliation evidence and real execution statistics before scaling capital. | Known gap |
